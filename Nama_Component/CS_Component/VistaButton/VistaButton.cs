...
namespace CS_Component
{
    ...
    public partial class VistaButton : UserControl
    {
        ...
        private void DrawHighlight(Graphics g)
        {
            ...
            //Pen inlinePen = new Pen(Color.White);  //Light-blue
        ...
        //Draws the button when the mouse is over it
        private void DrawHoverState1(Graphics g)
        {
            GraphicsPath outerRegion = RoundedRect(this.Width - 1, this.Height - 1, 3);
            GraphicsPath innerRegion = RoundedRect(this.Width - 3, this.Height - 3, 2);
            //----Shift the inner region inwards
        ...
        private void DrawForeground(Graphics g)
        {
            ...
            Color textColor1 = this.ForeColor;

            if (!this.Enabled)
                textColor = SystemColors.GrayText;

            SolidBrush textBrush = new SolidBrush(textColor);
            SolidBrush textBrush1 = new SolidBrush(textColor1);
            //------------------------------------------
            ...
            //g.DrawString(headerText, this.Font, textBrush, Width - headerLayout.Width - imageSize.Width - 5, totalRect.Top + offset);
            //g.DrawString(DescriptionText, descriptFont, textBrush1, Width - descriptLayout.Width - imageSize.Width - 5, totalRect.Bottom - (int)descriptLayout.Height + offset);

            //if (image != null)
            //{
            //    if (this.Enabled)
            //        //g.DrawImage(image, new Rectangle(imageLeft + offset, imageTop + offset, imageSize.Width, imageSize.Height));
            //        g.DrawImage(image, new Rectangle(Width - imageSize.Width, (Height/2)-(imageSize.Height/2), imageSize.Width, imageSize.Height));
            //    else
            //    {
            ...
            //    }
            //}
            g.DrawString(headerText, this.Font, textBrush, imageLeft + imageSize.Width + 5 + offset, totalRect.Top + offset);
            g.DrawString(DescriptionText, descriptFont, textBrush1, imageLeft + imageSize.Width + 5 + offset, totalRect.Bottom - (int)descriptLayout.Height + offset);

            // عکس سمت چپ
            if (image != null)
            {
                if (this.Enabled)
                    g.DrawImage(image, new Rectangle(imageLeft + offset, (Height / 2) - (imageSize.Height / 2), imageSize.Width, imageSize.Height));
                else
                {
                    if (grayImage == null)
                        grayImage = GetGrayscale(image); //generate grayscale now
                    g.DrawImage(grayImage, new Rectangle(imageLeft + offset, (Height / 2) - (imageSize.Height / 2), imageSize.Width, imageSize.Height));
