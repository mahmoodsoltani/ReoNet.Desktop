...
namespace Cs_Component
{
    ...
    /// </para>
    /// </remarks>
    public class SectionText : Cs_Component.ReportSection
    {
...
//NDA : rev 0.53
		public static StringAlignment ConvertAlign(VerticalAlignment stringAlign) {
			StringAlignment strAlign = StringAlignment.Near;
			switch (stringAlign) {
...
// NDA : rev 0.53
			stringFormat.LineAlignment= ConvertAlign(this.VerticalAlignment);
			return stringFormat;
        }
...
// NDA 0.53
			Region org_region= g.Clip;
			try {
				g.Clip = new Region(textLayout);
...
// NDA: rev. 0.53: removed to avoid infinite loop when the char dimension is taller than the containing size: always draw the text !
...
//			if (charsFitted < textToPrint.Length)
			if ((charsFitted < textToPrint.Length)&& (textLayout.Height> textFont.GetHeight(g)))
            {
                // it doesn't all fit.
