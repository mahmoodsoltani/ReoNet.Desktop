...
Public Class Tree_Vw
    ...
    Public Class MyDataItem
        Public Property Srl As String
        Public Property Srl_Parent As String
    ...
    Public Sub Fill(ByVal dt As DataTable)
        ...
        ' 2. یک دیکشنری برای نگهداری ارجاع به هر TreeNode ایجاد کنید.
        ' کلید (Key) برابر با Srl هر آیتم خواهد بود.
        Dim nodeMap As New Dictionary(Of String, TreeNode)()

        ' 3. تمام آیتم‌ها را به گره‌های TreeView تبدیل کرده و در نقشه ذخیره کنید.
        ' این کار به ما امکان می‌دهد تا به سرعت گره والد را پیدا کنیم.
        For Each item As DataRowView In dv
            Dim newNode As New TreeNode("")
            newNode.Text = item("name")
            newNode.Srl_Parent = item("srl_parent")
            newNode.Srl = item("srl") ' ذخیره Srl در Tag برای ارجاع آسان
            nodeMap.Add(newNode.Srl, newNode)
        Next

        ' 4. یک بار دیگر روی تمام آیتم‌ها حلقه بزنید تا ساختار سلسله‌مراتبی را ایجاد کنید.
        For Each item As DataRowView In dv
            Dim currentTreeNode As TreeNode = nodeMap(item("srl"))

            If nodeMap.ContainsKey(item("srl_parent")) Then
                ' گره والد را از نقشه پیدا کنید.
                Dim parentNode As TreeNode = nodeMap(item("srl_parent"))
                ' گره فعلی را به عنوان فرزند به والدش اضافه کنید.
                parentNode.Nodes.Add(currentTreeNode)
            Else
                ' اگر والد پیدا نشد، این گره یک ریشه است.
                TreeView1.Nodes.Add(currentTreeNode)
            End If
        Next

        ' 5. TreeView را از حالت تعلیق خارج کنید.
        TreeView1.EndUpdate()
        'TreeView1.BeginUpdate()
        'TreeView1.Nodes.Clear()
        'Dim rootNodes As New List(Of DataRowView)
        'For Each row As DataRowView In dv
        '    If row("Srl_Parent").ToString() = "0" Then
        '        rootNodes.Add(row)
        '        Exit For
        '    End If
        'Next
        'Dim childNodesByParent As New Dictionary(Of String, List(Of DataRowView))

        'For Each row As DataRowView In dv
        '    Dim parentSrl As String = row("Srl_Parent").ToString()
        '    If Not childNodesByParent.ContainsKey(parentSrl) Then
        '        childNodesByParent.Add(parentSrl, New List(Of DataRowView))
        '    End If
        '    childNodesByParent(parentSrl).Add(row)
        'Next
        'For Each rootRow As DataRowView In rootNodes
        '    Dim rootNode As New TreeNode("")
        '    rootNode.Text = rootRow("Name").ToString()
        '    rootNode.Srl = rootRow("Srl").ToString()
        '    rootNode.IsRootNode = True
        '    TreeView1.Nodes.Add(rootNode)

        '    findAndAddOptimized(rootNode, rootNode.Srl, childNodesByParent)
        'Next

        'Me.TreeView1.EndUpdate()
    End Sub

    Private Sub findAndAddOptimized(ByVal parentNode As TreeNode, ByVal parentSrl As Integer, ByVal childNodesByParent As Dictionary(Of String, List(Of DataRowView)))
        If childNodesByParent.ContainsKey(parentSrl) Then
            For Each childRow As DataRowView In childNodesByParent(parentSrl)
                Dim newTreeNode As New TreeNode("")
                Try
                    newTreeNode.Text = childRow("Name").ToString() + " - " + childRow("Code").ToString()
                Catch ex As Exception
                    newTreeNode.Text = childRow("Name").ToString()
                End Try

                newTreeNode.Srl = CInt(childRow("Srl")) ' Use the Tag property to store data like Srl

                parentNode.Nodes.Add(newTreeNode)
                findAndAddOptimized(newTreeNode, newTreeNode.Srl, childNodesByParent)
            Next
        End If
    End Sub
    ...
    Public Sub DeleteSelectedNode()
        Try
            If CType(TreeView1.SelectedNode, VB_Component.TreeNode).IsRootNode = True Then
                MessageBoxFa.Show("This Node cannot be deleted.")
                Return
            End If
            If MessageBoxFa.Show("Are you sure?", "", MessageBoxButtons.YesNo) = System.Windows.Forms.DialogResult.Yes Then
                If MessageBoxFa.Show("Once this information is deleted, it cannot be recovered. Would you like to continue?", "", MessageBoxButtons.YesNo) = System.Windows.Forms.DialogResult.No Then
                    Return
                Else
            ...
            End If
        Catch
            MessageBoxFa.Show("Please select an operation")
            Return
        End Try
        'Fill(dt)
    End Sub
